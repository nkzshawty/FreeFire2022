...
function memberFromAccount(account, groupId, seat, role = ROLE_MEMBER) {
  ...
  return {
    ...
    account_role: Number(account.role) || 0,
...
function playerInfo(m, mapId) {
  return {
    ...
    role: Number(m.account_role) === 100 ? 100 : (m.role || ROLE_MEMBER), pin_id: Number(m.account_role) === 100 ? 100 : (m.pin_id || 0), rank: m.rank || 0, ranking_points: 0,
