...
function buildAccountInfoBasic(acc) {
  ...
  return {
    ...
    role: Number(acc.role) === 100 ? 100 : (Number(acc.role) || 0),
...
function accountToBasic(acc) {
  ...
  return {
    ...
    rank: acc.rank || 1,
    ranking_points: acc.ranking_points || 0,
    role: Number(acc.role) === 100 ? 100 : (acc.clan && acc.clan.role && acc.clan.role !== 'none' ? 1 : 0),
    pin_id: Number(acc.role) === 100 ? 100 : (acc.pin_id || 0),
    has_elite_pass: !!acc.elite_pass,
