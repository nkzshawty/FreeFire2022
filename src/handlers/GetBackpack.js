...
function handleGetBackpack(reqObj, ctx) {
    ...
    if (epBadgeCount > 0) {
    // Infinite custom-room cards: the client renders these as room-card inventory items.
    for (const roomCardId of [600000001, 600000002]) {
        items = items.filter(item => item.id !== roomCardId);
        items.push({ id: roomCardId, cnt: 999999999, expire_time: 0, left_use_times: -1, history_owned_cnt: 999999999, item_status: 1 });
    }
        items.push({
            id: epBadgeId,
            cnt: epBadgeCount,
